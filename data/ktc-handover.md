...
### Tab "คณะกรรมการ" แสดงข้อมูลของ "คณะผู้บริหาร" แทน
Root Cause: หน้าคณะกรรมการอยู่ในสถานะ Draft เพราะผู้แก้ไขกด Unpublish ก่อนกด Save Draft ทำให้เนื้อหาเวอร์ชันที่เคย Publish อยู่ก่อนหน้าถูกถอดออกไปด้วย หน้าจึงไปแสดงเนื้อหาของ tab อื่น (คณะผู้บริหาร) แทน (KTC-30)
Resolution: ระบุสาเหตุจากลำดับการกด Unpublish/Save Draft ที่ผิด แก้ไขเนื้อหาให้กลับมาแสดงถูกต้อง (ดูวิธีตรวจสอบ Version history ในหัวข้อ "ข้อมูล tab คณะกรรมการไม่แสดง" ด้านบน — เคสนี้เป็นรูปแบบเดียวกัน)

### ฟิลด์ "Dev Json" ในหน้าแก้ไข Article สร้างความสับสน
Root Cause: ฟิลด์ "Dev Json" เป็นผลตกค้างจากการ migrate ข้อมูลเดิม เวลาสร้าง Article ใหม่จะ duplicate ค่ามาจาก Article อื่นโดยอัตโนมัติ ทั้งที่ไม่ควรปรากฏในหน้านี้และไม่มีผลต่อการทำงานใดๆ (KTC-49)
Resolution: ชี้แจงผู้ใช้ว่าฟิลด์นี้ไม่ต้องแก้ไขและไม่มีผลกระทบ ทีมวางแผนซ่อนฟิลด์นี้ออกจากหน้า CMS เพื่อไม่ให้สับสนอีก

### สารบัญบทความ (Table of Contents) ดึงหัวข้อจากฟอร์มที่ฝังอยู่ในหน้ามาแสดงด้วย
Root Cause: ระบบสร้างสารบัญจาก tag H2 ในหน้า แต่ฟอร์ม (merchant/promotion) ที่ฝังอยู่ในหน้าก็มี H2 อยู่ด้วย ทำให้หัวข้อของฟอร์มถูกดึงมาแสดงในสารบัญบทความอย่างผิดที่ (KTC-65)
Resolution: ปรับให้สารบัญสนใจเฉพาะ H2 ที่อยู่ใน Content จริงเท่านั้น ไม่รวม H2 จากฟอร์มที่ฝังเข้ามา deploy แก้ไขเรียบร้อยแล้ว

### กด Publish/ลบ ใน CMS ไม่ได้ เพราะ error จากการเช็ค Log ฝั่ง CRV
Root Cause: เกิด error จากการเช็ค log ฝั่ง CRV (ฝั่ง Muze) ทำให้กด save/publish/ลบ ใน CMS ไม่ได้ ระหว่างตรวจสอบยังพบว่าข้อมูล Title (HTML) ของหน้าฟอร์มบางหน้าหายไป/ผิดพลาดด้วย (KTC-87)
Resolution: แก้ไขฝั่ง save จนกด save/publish/ลบ ได้ตามปกติ ส่วนข้อมูล Title ที่หายไป query และแก้ไข manual ทีละหน้าฟอร์ม

### สิทธิ์ของ Role บางประเภทถูกจำกัดจนอัพโหลดรูปภาพไม่ได้
Root Cause: บัญชีของทีม OA (role = OA team) ไม่มีสิทธิ์ upload รูปภาพใน CMS ทำให้เปลี่ยนรูปหน้า OA Landing Page แล้วอัพโหลดไม่ผ่าน (KTC-131)
Resolution: ปรับ access ของ role OA team ให้มีสิทธิ์ upload รูปภาพได้แล้ว

### Keyword field ใน Dev Config สะกดไม่ตรงกับที่ Frontend เรียกใช้ ทำให้แสดงผลผิด/ซ้อนกัน
Root Cause: Dev Config ของ page system ตั้งชื่อ keyword เป็น `productTypes` แต่โค้ดจริงต้องการ `productType` ทำให้การแสดงผล product type บนหน้าเว็บและ CMS ผิดพลาด รวมถึงหน้า Preview มีเนื้อหาซ้อนทับกัน (KTC-117)
Resolution: แก้ wording จาก `productTypes` เป็น `productType` ข้อมูลแสดงถูกต้องตามแต่ละหน้าโปรโมชั่นแล้ว ส่วนปัญหา Preview ซ้อนกันทดสอบผ่านบน UAT รอ deploy ขึ้น PROD รอบถัดไป

### Slug/Parent Path ของหน้า Investor Relations หายไปทุกครั้งที่มี Deploy
Root Cause: หน้า Investor Relations บางหน้าตั้งค่า parent/slug แบบ virtual breadcrumb ซึ่งค่าพวกนี้จะหลุด/รีเซ็ตทุกครั้งที่มีการ deploy หรือแก้บั๊กอื่นในระบบ ทำให้ URL ใช้งานไม่ได้จนกว่าจะเข้าไป Save ที่ CMS ใหม่ (KTC-188)
Resolution: Workaround ระยะสั้นคือเข้าไปตั้งค่า parent ใหม่ เติม path เดิมกลับเข้าไป และเพิ่ม Breadcrumb Virtual Parent 2 ชั้นตามตัวอย่างหน้าอื่น ทุกครั้งที่เจอปัญหา — ยังไม่มี long-term fix ที่สรุปแล้ว (สถานะ: รอทีมยืนยันแนวทางต่อ)

### บทความใหม่ไม่มี Class จาก Migration ทำให้กล่อง Banner สมัครบัตรแสดงผลผิด
Root Cause: บทความเก่าที่ migrate มาจะมี class "content-migration-container article-migration-container" ติดมาด้วย ทำให้เข้าใจผิดว่าบทความใหม่ต้องใส่ class นี้ด้วย Banner ถึงจะแสดงถูก แต่จริงๆ สาเหตุคือ Custom CSS ของบทความนั้นตั้งค่าไม่ครบ ไม่เกี่ยวกับ class จาก migration (KTC-196)
Resolution: ปรับ Custom CSS ของบทความที่มีปัญหาให้รองรับการแสดง Banner ถูกต้อง — บทความใหม่ไม่จำเป็นต้องใส่ class จาก migration แต่ต้องเช็ก Custom CSS ที่ผูกกับ component แทน

### หน้า Investor Relations ภาษาอังกฤษ แสดงชื่อเดือนเป็นภาษาไทย
Root Cause: หน้า /en/investor-relations ตั้งค่า date locale ในโค้ดผิด ทำให้เดือนแสดงเป็นภาษาไทยแม้เข้าเวอร์ชันภาษาอังกฤษ ต้องแก้ที่โค้ดโดยตรง ไม่ใช่การตั้งค่าใน CMS (KTC-199)
Resolution: ยังไม่มีข้อสรุปสุดท้ายในเคส — ระบุ root cause แล้ว รอผู้แจ้งเปิด IRTASK เพื่อดำเนินการแก้โค้ดต่อ (สถานะ Escalated)
...
### ฟอร์มโปรโมชั่นที่ Migrate มาจากระบบเดิม ไม่ได้อัปเดตให้ส่งค่า CRV ครบเหมือนฟอร์มใหม่
Root Cause: หลายแคมเปญ (เช่น DN6, T25, FSF/FLC/FKP/TP25) พบข้อมูลลูกค้าที่ลงทะเบียนผ่านเว็บบางรายการส่งค่าไป CRV ไม่ครบ สาเหตุคือช่วงที่ทีมแก้บั๊กให้ฟอร์มส่งค่า CRV ถูกต้อง ทีมแก้เฉพาะฟอร์มใหม่ ไม่ได้แก้ฟอร์มที่เป็นข้อมูล "Migration" จากระบบเดิมด้วย ทำให้ฟอร์ม migrate ทั้งหมดยังส่งค่าผิดต่อไป (ฟอร์มที่ยิงจาก KTC App ไม่กระทบเพราะฝั่ง App ไม่ได้ถูกแก้ไข) (KTC-26, KTC-33, KTC-39, KTC-44)
Resolution: deploy แก้ไขให้ครอบคลุมฟอร์ม migration ด้วยแล้ว ยืนยันว่ารายการหลัง deploy จะส่งค่าไป CRV ถูกต้อง — หมายเหตุ: บางฟิลด์ (เช่นวิธีผ่อนชำระ) ไม่ได้ตั้งเป็น required field ใน CMS โดยตั้งใจ (business logic ให้เลือกกรอกได้/ไม่กรอกก็ได้) จึงไม่มีค่าส่งไปหากลูกค้าไม่กรอก — กรณีนี้ไม่ใช่บั๊ก

### Highlight Banner ของโปรที่อยู่หลาย Category ดึง URL จาก Category ที่ 2 แทนที่จะเป็น Category แรก
Root Cause: โปรโมชั่นที่ผูกไว้มากกว่า 1 Category บน Highlight Banner (เช่นหน้า Promotion Listing) โค้ด query ดึง URL ผิดจุด คือดึงจาก Category ลำดับที่ 2 แทนที่จะดึงจาก Category แรก (KTC-178) — รูปแบบเดียวกับปัญหา URL ผิดใน "โปรโมชั่นที่คุณอาจสนใจ" ด้านล่าง แต่เกิดกับ Component คนละตัว
Resolution: แก้โค้ดให้ query ดึงจาก Category แรกเสมอ deploy ขึ้น PROD แล้ว (14 July 2026) ทดสอบยืนยันเรียบร้อย
...
### กล่องค้นหาที่หน้าเว็บกระตุกตอนโหลด (Mobile)
Root Cause: ระบบตรวจจับว่าเป็น mobile ได้ช้ากว่ารอบ render แรก ทำให้หน้าเว็บยังไม่ทันคำนวณขนาดจอ กล่องค้นหาจึงกระตุกเป็นจังหวะตอนโหลด (KTC-59)
Resolution: แก้ไขบน UAT และ deploy ขึ้น PROD เรียบร้อยแล้ว

### ตั้งค่า Key Message เป็น Algolia Searchable Attribute ไว้ที่ UAT แต่ลืม Sync มา PROD
Root Cause: เคยตั้งค่าฟิลด์ Key Message (เช่น ชื่อร้าน/แบรนด์ที่มีชื่อเรียกหลายแบบ ทั้งไทย/อังกฤษ/ย่อ) ให้เป็น Algolia searchable attribute ไว้ที่ UAT แต่ไม่ได้ sync การตั้งค่าเดียวกันมาที่ PROD ทำให้ค้นหาด้วยคำที่กรอกไว้ใน Key Message บนเว็บจริงไม่เจอโปรในลำดับต้นๆ (KTC-194)
Resolution: เพิ่ม Key Message เป็น searchable attribute ใน PROD ให้ตรงกับ UAT เรียบร้อยแล้ว
...
### Section บนหน้าเว็บแสดงผลไม่เรียงลำดับตอนโหลด/รีเฟรชหน้า เพราะ Block ที่มีข้อมูลเยอะ Render ช้ากว่า
Root Cause: หน้าปฏิทินกิจกรรมมี block RichText (ข้อมูลน้อย) อยู่ก่อน block DMC (ข้อมูล HTML เยอะกว่า) เมื่อโหลด/รีเฟรชหน้า block RichText render เสร็จก่อนจึงโผล่มาก่อน ทำให้ลำดับ section แสดงผลสลับกับที่ตั้งไว้ใน CMS ทั้งที่ไม่มี effect AOS เข้ามาเกี่ยวข้อง (KTC-134)
Resolution: แนะนำให้ custom CSS กำหนดความสูง (min-height) ของ block DMC ไว้ล่วงหน้า เพื่อลดอาการกระพริบ/สลับลำดับตอน render — ยังไม่มีการแก้ที่ระบบเรียงลำดับ section โดยตรง

### Splash Page แสดงไม่สม่ำเสมอเพราะติด Cache CDN
Root Cause: การทำงานของ Splash Page อยู่ฝั่ง Server และมีการทำ Cache CDN ครอบอยู่ ทำให้บางครั้ง splash ขึ้นบ้างไม่ขึ้นบ้าง (KTC-135)
Resolution: แก้ไขชั่วคราวด้วยการย้าย logic splash ไปทำงานที่ระดับหน้า Home และ Promotion Landing แทนเมนู Splash Page แต่พบผลข้างเคียงคือลูกค้าที่มาจากการค้นหาโปรโมชั่นบน Google แล้วเจอ splash จะถูกพาไป Homepage เสมอแทนที่จะไปหน้าโปรที่ค้นเจอ (journey สะดุด) ทีมจึง Unpublish splash ออกก่อน — ยังไม่มีข้อสรุปสุดท้าย (รอ solution ระยะยาว เช่นย้ายไป render ฝั่ง client)

### ใส่ Custom CSS ผิด Class ทำให้ซ่อน Tab/Chip หน้า Store List ไม่ได้
Root Cause: ใส่ custom CSS `.pm-store-list-chips{display:none}` เพื่อซ่อน tab หน้า promotion แต่ class นี้ไม่ใช่ selector ที่ควบคุมการแสดงผลจริงของ tab นั้น (KTC-150)
Resolution: class ที่ถูกต้องคือ `.tags-container{display:none !important;}` ใช้งานได้ตามต้องการ

### บาง Block แสดงผลผิดปกติหลัง Deploy — แก้ชั่วคราวด้วย Custom CSS ระหว่างรอแก้โค้ดถาวร
Root Cause: Announce Block ที่หน้า Home และ Modal ที่หน้า Reward แสดงผลผิดปกติหลัง deploy โดยยังไม่ทราบสาเหตุจากโค้ดหลังบ้านชัดเจนในทันที (KTC-78, KTC-86)
Resolution: ทำ custom CSS เฉพาะ block นั้นๆ ให้แสดงผลถูกต้องไปก่อนในระยะสั้น ตามด้วยแก้ไขโค้ดหลังบ้านและ deploy รอบถัดไป ยืนยันผลสำเร็จหลัง deploy ทั้งสองเคส

### ฟอนต์บางหน้าแสดงผิด ไม่ตรงตาม Design (เช่นไม่ใช่ Sarabun)
Root Cause: บั๊กการโหลดฟอนต์ (font loading bug) ของหน้าเว็บ ทำให้ฟอนต์ที่แสดงไม่ตรงกับที่ดีไซน์กำหนดไว้ (KTC-12)
Resolution: แก้ไขบั๊กการโหลดฟอนต์ ทดสอบบน UAT ก่อน แล้ว deploy ขึ้น production สำเร็จ
...
### ฟิลด์ประเภท Number ใน CMS ใส่ Comma คั่นหลักพันอัตโนมัติ ไม่เหมาะกับฟิลด์ที่ไม่ใช่จำนวนเงิน
Root Cause: ต้องการนำเครื่องหมาย "," ออกจาก field number เพราะ field ประเภท Number ของ CMS จะใส่ comma คั่นหลักพันให้อัตโนมัติ ซึ่งไม่เหมาะกับข้อมูลที่ไม่ใช่จำนวนเงิน เช่น รหัส/เลขที่ที่ต้องการแสดงเป็นตัวเลขล้วนไม่มี comma (KTC-3)
Resolution: สรุปว่าเป็น Change Request (ต้องพัฒนาเพิ่ม) ไม่ใช่บั๊ก — ระหว่างรอพัฒนาให้ใช้ Text field แทน Number field เป็นทางแก้ชั่วคราว ยังไม่มีข้อสรุปว่าจะพัฒนาฟีเจอร์นี้เพิ่มหรือไม่
...
### หน้าโปรโมชั่นบางหน้าขึ้น Error "Stream Timeout"
Root Cause: หน้า Promotion detail และหน้า listing บางหน้าขึ้น error stream timeout ขณะโหลด ทำให้เข้าเนื้อหาไม่ได้ ยืนยันว่าเป็นบั๊กฝั่งระบบ ไม่ใช่ปัญหาที่ตัวเนื้อหา (KTC-18, KTC-31) — คนละเคสกับหัวข้อ "Stream timeout บนหน้าเว็บ" ด้านบนแต่เป็นรูปแบบเดียวกัน (เกิดซ้ำ)
Resolution: แก้ไขและ deploy (26 ก.พ. 2026) ทดสอบซ้ำไม่พบหน้า stream timeout อีก

### กดลิงก์ Offer Page จากการแจ้งเตือนใน App แล้วหมุนโหลดค้าง
Root Cause: กดลิงก์ offer page จาก push notification สิทธิพิเศษใน App แล้วหน้าโหลดค้างไม่จบ ตรวจสอบพบว่า API เส้น get profile (`getUserInfo`) เกิด Gateway Timeout เนื่องจากเรียก SOAP API ของฝั่ง KTC ไม่สำเร็จ (KTC-22)
Resolution: แจ้งทีม KTC ให้ตรวจสอบและ restart service ฝั่ง backend หลัง restart แล้วเข้าลิงก์ offer page ได้ตามปกติ

### Stream Timeout ที่หน้า Promotion เป็นระยะ เพราะข้อมูล Store List ในฐานข้อมูลใหญ่เกินไป
Root Cause: ข้อมูล Store List ในฐานข้อมูลมีขนาดใหญ่เกินไป ทำให้ query หนักและเกิด stream timeout เป็นระยะๆ ที่หน้าโปรโมชั่น (KTC-98)
Resolution: แก้ไขระยะสั้นโดยลบข้อมูล Store List ออกจากฐานข้อมูลเพื่อลดภาระ query ส่วนระยะยาวทีม Dev ยังดำเนินการหาทางแก้ต่อเนื่อง ยืนยันหลัง deploy รอบถัดไปว่าไม่พบ stream timeout อีก

### อีเมล Notification อัตโนมัติไม่ถูกส่งเมื่อ SMTP เชื่อมต่อไม่ได้ในรอบเวลานั้น
Root Cause: ระบบส่งอีเมลแจ้งเตือนอัตโนมัติในรอบเวลา 9 โมงเช้าไม่สามารถเชื่อมต่อ SMTP Server ของ KTC ได้ ทำให้อีเมลรอบที่ไม่มีไฟล์แนบในวันนั้นไม่ถูกส่งออกไปเลย ขณะที่รอบ 15:00 น. เชื่อมต่อได้ปกติ (KTC-115)
Resolution: ตรวจสอบ log ยืนยันช่วงเวลาที่ SMTP หลุด ส่งเรื่องต่อให้ทีมเครือข่ายของ KTC ตรวจสอบ — ไม่มีการเพิ่ม retry/alert ในระบบส่งอีเมลเอง เป็นการรายงานปัญหาให้ฝั่ง infra ตรวจสอบต่อ

### เว็บไซต์เข้าไม่ได้ชั่วคราวจาก CPU Spike ระยะสั้น
Root Cause: ตรวจสอบ log พบ CPU peak ในช่วงเวลาสั้นๆ (ไม่ถึง 1 นาที) ที่ทำให้ผู้ใช้เข้าเว็บไซต์ไม่ได้ชั่วขณะ (KTC-146)
Resolution: ยืนยันจาก log ว่าเป็น CPU spike ระยะสั้นตามเวลาที่แจ้ง — ไม่ได้ระบุ root cause เชิงลึกของสาเหตุ CPU spike หรือมาตรการป้องกันเพิ่มเติมในเคสนี้

### หน้าเว็บพังหลังปล่อยทิ้งไว้ (Idle) แล้วกลับมาดูใหม่
Root Cause: เปิดหน้าเว็บทิ้งไว้แล้วไปทำอย่างอื่น พอกลับมาหน้าเว็บ broken ตั้งสมมติฐานว่าอาจเกิดจาก network ภายในองค์กรของ KTC บล็อกการเรียก API บางเส้น (KTC-75)
Resolution: ยังไม่มีข้อสรุปสุดท้ายในเคส — ผู้ใช้ไม่ได้ทดสอบซ้ำด้วยเน็ตนอกองค์กรและไม่ตอบกลับเพิ่มเติม ทีมจึงปิดเคสโดยไม่ได้ยืนยัน root cause ที่แท้จริง

### เพิ่ม Script (Analytics) ผ่าน Site Meta แล้วไม่ขึ้นผลทันที
Root Cause: เพิ่ม analytics script (เช่น ahrefs analytics.js) ผ่านเมนู Site Meta ในหลังบ้าน แต่ยังไม่เห็นผลบนหน้าเว็บทันที ซึ่งไม่ใช่บั๊ก แต่เกิดจากรอบการรัน deploy/cache ที่ยังไม่อัพเดท (KTC-96)
Resolution: Inspect หน้าเว็บพบว่า script ถูกฝังเรียบร้อยแล้ว หลังรอคิว deploy/cache สักระยะข้อมูลอัพเดทตามปกติ

## Partner / Merchant Integration

### ลูกค้าใช้โปรโมชั่น/ลิงก์จองผ่านพันธมิตรภายนอกไม่ได้ แต่สาเหตุอยู่ที่ระบบของพันธมิตร ไม่ใช่เว็บ KTC
Root Cause: พบ 2 เคสแยกกัน (1) กรอกโค้ดโปรโมชั่นจองตั๋วการบินไทยแล้วขึ้น "Code invalid" (KTC-189) (2) กดลิงก์จองผ่าน Thai Lion Air แล้วเจอหน้า 404 ของเว็บ Lion Air เอง (KTC-200) ทั้งสองเคสตรวจสอบแล้วพบว่า KTC website ทำงานปกติ (ลิงก์ redirect ถูกต้อง) ปัญหาอยู่ฝั่งระบบ/เว็บไซต์ของพันธมิตรเอง ไม่มี log ฝั่ง KTC เนื่องจากไม่ใช่ระบบของ KTC
Resolution: KTC-189 ทีม MKT ประสานงานกับพันธมิตรโดยตรงจนกลับมาใช้งานได้ปกติ KTC-200 แจ้งปิดเคสและแนะนำให้ติดต่อฝั่งพันธมิตรโดยตรงเพื่อตรวจสอบเว็บไซต์ของเขา เนื่องจากไม่ใช่ปัญหาที่แก้ไขได้จากฝั่ง KTC
